...
function loadSkulpt() {
  ...
  skulptLoadPromise = new Promise((resolve, reject) => {
    ...
    // Load Skulpt from a CDN (will be cached by service worker)
    const script = document.createElement('script');
    script.src = 'https://cdn.jsdelivr.net/npm/skulpt@1.3.0/dist/skulpt.min.js';
    script.async = true;
    script.onload = () => {
      skulptLoaded = true;
      configureSkulpt();
    ...
    read: (filename) => {
      ...
      if (filename === 'sys' || filename === 'math' || filename === 'random' ||
        ...
        return window.Sk.misceval.promiseToSuspension(
          fetch(`https://cdn.jsdelivr.net/npm/skulpt@1.3.0/dist/${filename}.js`)
            .then(r => r.text())
